...
- 면허제한 그룹(`limGroups`): lic_map `grp` {공고ID: [[그룹, lic 번호, mfn 번호|-1]]} 또는 실시간 `b.reqLim`. **같은 그룹은 모든 줄(면허 보유 + 주력분야 `mfRowPass`) 충족, 그룹끼리는 택일.** 그룹 정보가 없으면 예전 방식(하나라도 겹치면). 수집기는 처음 한 번 30일치 면허제한을 다시 받는다(`meta.lim_scan`).
