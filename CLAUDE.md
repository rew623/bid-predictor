...
개인 데이터는 localStorage + (구글 로그인하면) Firebase Firestore 동기화(`Cloud`, 프로젝트 bidproject-452e5, 2026-09-26).

## 작업 규칙
- **요청한 부분만 수정**한다. 관련 없는 리팩터링·디자인 변경 금지.
- **하단 탭 6개 유지**: 🏢 우리 공고(첫 화면) / 🔎 공고 검색 / ⭐ 내 투찰 / 📊 분석 / 👥 업체 / ⚙️ 설정 (PC에서는 좌측 사이드바). 2026-09-26 사용자 요청으로 재구성(업체 검색은 같은 날 요청으로 독립 탭 `view-corp`) — 섹션 id 는 view-home / view-bids / view-watch / view-corp / view-predict(+ view-paper·view-stats, 분석 탭 위 '금액 분석·모의 투찰·통계' 전환) / view-settings.
- 조달청 브라우저 호출(`liveCall`)은 동시 5건까지(`LIVE_MAX`) — 더 보내면 '초당 서비스 요청제한 횟수 초과'(2026-09-27). 그 오류는 0.5·1·2·4초 쉬고 재시도.
...
## 파일 지도
```
index.html            화면 뼈대 (home·bids·watch·predict·stats·settings section + nav 5개)
...
- 하루 호출 한도: 서비스별 `API_DAILY_LIMIT`(코드 기본 950, 2026-09-26 운영계정 승인 뒤 워크플로에서 9500 — 저장소 변수 `API_DAILY_LIMIT` 로 바꿈). 호출 수는 `meta.api.calls` 에 날짜별로 기록, 넘으면 저장 후 다음 날 이어서.
...
- 더비스 '참여가능공고'와 차이(2026-09-26 대조): 더비스는 주력분야 제한(예: 습식·방수)을 안 거르는 듯 — 우리는 설정의 주력분야로 거름. 회사 이력에 도장 주력 제한 공고 정상 투찰(65/98)이 있어 도장 주력은 보유. 한국남동발전 등 나라장터 밖 공고는 우리 데이터에 없음.
- 더비스(유료 분석 서비스) 비교(2026-09): 공사 제시가가 투찰 사정률 100.4~100.9% 수준. 고정 100.5/100.8 역검증 공정 기대 대비 ×0.87/0.73 — 사정율보다 한참 높게 넣는 방식은 기대보다 못함. 추천 체크에 하한 미달 비율(`sb`) 설명.
...
- **지역 전용 추천**(`local_models` → model.json `local` {"시도|시군": {label, small, seg:{small|big:{use:'sido'|'sgg'|null, val:{n,fair,national,sido,sgg}}}, curves:{sido|sgg:{small|big:{예가범위: 곡선}}}}}, 설정 `scripts/local_models.json`): 그 지역 공고를 같은 규모(예상 참가 small=50 미만/이상)끼리 시·도·시·군 곡선으로. 매일 최근 12개월 표본외로 전국 모델과 **같은 공고끼리** 비교 — 엇갈린 공고(지역만 낙찰 b / 전국만 낙찰 c)로 한쪽 부호검정 p < 0.05÷4(본페로니, 시도·시군 × 규모) 그리고 +10%↑일 때만 `use` (2026-09-27 강화: 예전 '+2건·+10%'는 춘천 small 시군 8 vs 전국 6(p 0.36, 우연)을 채택할 뻔함). val 에 `{lv}_b/_c/_p`. 앱 `modelPredict`가 그 공고(sido|sgg)·구간이 use면 그 곡선으로 바꾸고 카드·예측 화면에 `areaText` 표시, 확률은 `areaLift`. 2026-09-26 첫 판정: 춘천 small 151건 전국 7/도 7/시 7, big 260건 4/5/2 → 둘 다 전국 유지. (시험 스크립트로 2025-07부터 보면 강원 같은 규모 20 vs 전국 17 — 기간 따라 흔들리는 수준.)
...
`{연도}/{월}.json` {"v":2, month, items:[{id(D-·F-·N-·NF-), kind(물품|용역|시설), nm, org, cm, dm, bm, date, base, budget, rng, floor, plan, sr, cnt, amt, rate, win, winBiz, p, r:상위 30곳(+WATCH_BIZ) [[순위, corps 인덱스, 금액, 비고?]], k(순위 있는 업체 수), c:참가 업체 전원 corps 인덱스(순위 순), x:투찰률×1000 앞 값과의 차이(첫 값 그대로)}]} — c·x 는 최근 `FULL_MONTHS`(12)개월만(24개월 전원은 약 570MB). 업체 표는 모든 달이 같이 쓰는 `corps.json` {"v":2, corps:[[업체명, 사업자번호, 대표자?]]}. 1건 약 3.4KB, 24개월 약 160MB 예상. `meta.json` {updated_at, files:{"YYYY-MM":[경로]}, counts, total, backfill}, `index.json` {v:2, done, fail} — v1(연도 파일·상위 30곳)이면 첫 실행에서 지우고 처음부터.
...
- **🏢 우리 공고 탭**(첫 화면, `view-home`, 대시보드 `renderMine`; 업체 정보가 있으면 앱을 열면 여기, 없으면 공고 검색): 설정의 업체 정보로 bids.json 진행중 공고 중 `eligibility` 통과(면허 정보 없는 공고는 제외)만 — 검색 없이. 요약(참가 가능·오늘 마감·7일 안·기초금액 공개·내 투찰 개찰 대기), 2주 투찰 마감 달력(날짜 누르면 그날만, `Mine.day`), 마감일별 묶음 목록, 정렬(마감·낙찰확률·기대 수주액 `bp.mineSort`). 서비스키가 있으면 그룹 정보 없는 공고의 면허제한을 조회해 다시 거른다.
  우리 공고 탭에 들어올 때마다 '🏗 공사 · 우리 시·도'로 시작, 공사 버튼 = 우리 시·도, 다른 업무 버튼 = 전체 지역(2026-09-26 요청).
  지역 버튼(`Mine.area`, `bp.mineArea`): 전체 지역 / 우리 시·도 / 우리 시·군 — 공사 현장(b.sido·b.sgg) 기준. 참가 제한 없는 전국 공고는 다른 지역 현장도 참가 가능으로 나오기 때문.
  필터 버튼(`mineChipDefs`, 더비스식): 전체 / 보유 면허 / 보유 주력분야(그 대업종 공고 중 주력분야를 안 걸었거나 그 주력분야를 건 것) + 뺀 공고. 카드 "목록에서 빼기"/"되돌리기" = `Hidden`(`localStorage bp.hiddenBids`, 이 기기에만).
  근거(2026-09-26, 금석건설 실제 이력): 공사 58건 참가 중앙값 386곳 → 공정 기대 낙찰 0.33건, 물품 153건 참가 중앙값 3,751곳 → 0.17건(참가 500곳 초과 125건은 합쳐 0.04건). **사용자는 참가 가능한 공고에 전부 넣는다(물품 특히) — '참가 적은 공고 고르기'는 권하지 말 것**(2026-09-26 요청으로 🎯 버튼·안내 삭제). 개선은 1건당 금액(추천값)에서.
...
- **우리 업체**(설정, `Company` → `localStorage bp.company` {name, sido, sgg, lics[], biz, goods:{small 소기업·소상공인, sme 중소기업, women 여성기업, direct 직접생산, inds:[등록 업종]}}) — goods 는 물품 참가 판정용(물품 우리 공고 구현 예정): `eligibility(b)` = 참가가능지역(rgn, 비면 제한 없음; "강원특별자치도"는 시·도 전체, "… 춘천시"는 그 시·군만) + 면허(겹치면 가능, 공고 면허 정보 없으면 '확인 필요'). 입찰공고 두 모드에 "참가 가능한 공고만"(`bElig`/`lElig`), 카드 태그 `eligTag`.
...
- 업체 보기: 대표자·주소·전화, 강원 1순위 / **강원 최종 낙찰**(낙찰 목록 낙찰자 = 1순위 포기·적격 탈락으로 올라온 경우 포함, 더비스 '최종낙찰'), 🏆 최종 낙찰 이력(corpw: 기초금액·사정율·낙찰률 기본 표시, '개찰 순위·공고 보기'를 펼치면 강원 개찰 상세/국방 월 파일에서 상위 10곳 + 공고 원문 링크). 검색은 업체명·대표자·사업자번호. 우리 개찰 결과도 낙찰 목록 winBiz 가 우리면 '🏆 최종 낙찰'.
...
- 카드(`bidCard`): 한눈에 = 지역·수의/정정·**요구 면허 짧은 이름**(`licShortTags`/`LIC_SHORT`, 더비스식 금속지붕(대)·도장습식석공(대), 우리 면허 ✓)·요구 주력분야·참가 가능 + **추천 투찰가(크게)·낙찰확률·예상 참가**, 나머지(요구 면허·주력·예가·하한·사정률·표본 설명)는 '▸ 자세히'(`details.b-more`). 버튼 '💰 투찰금액 분석'=분석 탭. 설정의 데이터 현황·수집·대조·역검증·직접 검증은 '🔧 고급'(`details.adv`)에 접어 둠.
- **📝 투찰 등록**(`registerBid(공고, 금액, 사정률, by)`): 관심공고 저장 + `joined` + `myBid` 채움 → 내 투찰 '투찰 중'으로 이동하고 그 카드 강조(`flashId`). 버튼: 공고 카드 '📝 추천가로 투찰 등록'(`data-quickbid`, quickPredict 금액), 분석 추천 카드 '📝 추천가로 투찰 등록', 후보 표 '적용'(`data-pick-sr` = 그래프에 초록 '선택' 선 + `#pickBox` 에 금액·'📝 이 금액으로 투찰 등록')·'계산기로'(`data-use-sr`), 계산기 결과 '📝 {최종금액}으로 투찰 등록'. 분석 화면 버튼은 공고를 골라 들어왔을 때(`P.notice`)만.
- **추천가였다면**(`recIfBid`, 카드 b-pred 칸): 개찰 끝난 공사 공고에 '낙찰가'(낙찰 기록 amt → 조달청 1순위) + '추천가였다면 k위 / N+1곳 · 🏆 1위 · 하한 미달'만(긴 개찰 결과 줄은 '자세히'로) — 조달청 개찰 결과(`fetchLiveResults` → `b.openRes`, 실시간 검색 10건씩) 또는 수집 낙찰+개찰 상세로 `judgeBid`. 추천값은 지금 모델(그 공고가 학습 기간에 들어 있을 수 있어 역검증과 다름).
...
- 사용자는 이 앱의 추천값으로 실제 입찰한다. 모든 개선은 표본외 역검증(낙찰 수 ÷ Σ1/참가수)으로 확인한 것만 넣고, 우위가 없으면 없다고 말한다. 공고는 고르지 않고 다 넣으므로 1건당 금액 우위만 의미가 있다.
...
- **유료 분석 서비스(더비스) 제시가 비교**: 개찰 뒤 실제 사정율·1위와 대조해 어느 쪽 추천이 맞았는지 쌓는다(사용자가 제시가를 더 보내 줌).
  | 공고 | 개찰 | 더비스 금액 / 투찰 사정률 | 우리 추천 x (예상 참가) |
  |---|---|---|---|
  ...
  더비스식(공사 100.4~100.8% 고정)은 역검증 ×0.66~0.82 로 평균보다 못했다(위 모델 근거 참고). 단건 결과로 결론 내지 말 것.
- 수집 워크플로는 GitHub API(workflow_dispatch, `quick_only`)로 바로 돌릴 수 있다. 하루 한도는 KST 날짜 기준.
- **2026-09-26 밤 상태 (새 대화에서 이어갈 것)** — 사용자 회사: 주식회사 금석건설(춘천, 대표 김경희, 사업자번호는 저장소 변수 `WATCH_BIZ` 에만). 물품 자격: 소기업·소상공인·중소기업·여성기업, 직생 없음, 등록 업종 동물용 의료기기 판매업·옥외광고업(사용자가 설정에 입력해야 함). 컨설팅 업체 추천 공고만 넣어 왔음(엑셀: 나라장터·국방·한국전력 31건, 컨설팅 '예상 범위' 있음 — 개찰 뒤 비교 예정, 파일은 대화 첨부라 private/ 에 없음).
  확인할 것: ① 대표자·주소(`업체정보` 단계, info_fill)가 채워졌는지 — 채워지면 업체 검색 '김경희'로 금석건설이 나와야 함 ② 국방 v2 수집 진행(data/d2b/meta.json backfill) ③ goods.json(나라장터 물품 진행 공고) 생성 ④ 모의 투찰 첫 채점(9/28~) ⑤ 개찰 상세 강원 잔여·전국 관심 면허 진행 ⑥ 저장소 크기(2026-09-26 35MB, 1GB 전에 기록 정리 방안).
  남은 일: 물품·국방 추천(데이터 모이면 역검증), 공고별 '이 공고에 올 경쟁사', 폰 달력 내보내기(.ics), 알림(Firebase 설정 필요), 한국전력·발전사 공고 데이터 가능 여부, 개찰 상세 확대(운영계정 승인됨 2026-09-26). 사용자 요청: 참가 적은 공고 고르기 권하지 말 것, 요금·저장 부담 되는 건 하지 말 것.
