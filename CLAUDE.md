...
- 카드(`bidCard`): 한눈에 = 지역·수의/정정·참가 가능 + **추천 투찰가(크게)·낙찰확률·예상 참가**, 나머지(요구 면허·주력·예가·하한·사정률·표본 설명)는 '▸ 자세히'(`details.b-more`). 버튼 '💰 금액 자세히'=분석 탭. 설정의 데이터 현황·수집·대조·역검증·직접 검증은 '🔧 고급'(`details.adv`)에 접어 둠.
