...
const Data = {
  ...
  url(path, ver=true){ return (window.DATA_BASE || 'data/') + path.split('/').map(encodeURIComponent).join('/') + (ver ? '?v=' + this.ver() : ''); },
...
function initServiceWorker(){
  if(!('serviceWorker' in navigator) || window.STABLE) return;   // 안정판은 서비스워커를 따로 쓰지 않음
  const show = (worker) => {
