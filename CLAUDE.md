...
scripts/corp_index.py 업체 색인 → data/corps.json (전국 낙찰자·개찰 상세 투찰·국방 상위, 하루 한 번, 약 3MB) — 앱 공고 검색 탭 '🏢 업체 검색'
...
- **🏢 업체 검색**(공고 검색 탭 세 번째 모드 `bidsMode='corp'`, `renderCorpSearch`/`renderCorpProfile`/`openCorp`): corps.json 에서 이름·사업자번호 검색 → 업체 보기 = 전국 낙찰 수 + 개찰 상세 지역(강원) 투찰 이력·1순위·공정 기대·하한 미달·**투찰 습관**(투찰 사정률 − 실제 사정율 분포)·자주 넣는 발주처·면허. ⭐ 관심 업체(`bp.corpWatch`, 동기화). 개찰 결과 상위 순위 표의 업체명(`data-corp`)을 누르면 바로 업체 보기. 대표자·주소는 없음(조달청 사용자정보 API 는 별도 활용신청 필요).
