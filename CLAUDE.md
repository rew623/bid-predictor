...
- **추천가였다면**(`recIfBid`, 카드): 개찰 끝난 공사 공고에 '🎯 추천가였다면 k위 / N+1곳 · 하한 미달 · 1위보다 ○원' — 조달청 개찰 결과(`fetchLiveResults` → `b.openRes`, 실시간 검색 5건씩) 또는 수집 낙찰+개찰 상세로 `judgeBid`. 추천값은 지금 모델(그 공고가 학습 기간에 들어 있을 수 있어 역검증과 다름).
